  ...
  describe 'when multiple tracks' do
    ...
    context 'and only the last two tracks of three are used' do
      ...
      before do
        competitors = [competitor1, competitor2, competitor3, competitor4, competitor5, competitor6, competitor7]
        expect(generator).to receive(:shuffle_competitors).and_return(competitors)
        generator.generate_qualification_round 1, 1, first_batch_time, minutes_between_batches, first_batch_track_number: 2, include_tracks: [2, 3]
      end

      it 'generates batches only to the given tracks' do
        expect(generator.errors).to eql []
        expect(race.qualification_round_batches.length).to eql 4 + 1 # 1: the second day batch
        verify_qualification_round_batch 1, first_batch_time, 1, 2
        verify_qualification_round_batch 2, first_batch_time, 1, 3
        verify_qualification_round_batch 3, second_batch_time, 1, 2
        verify_qualification_round_batch 4, second_batch_time, 1, 3
        verify_competitor competitor1, 1, 1
        verify_competitor competitor7, 4, 1
      end
