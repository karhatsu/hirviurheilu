...
describe RelayShootingPenaltiesAdjustmentQuickSave do
  before do
    @race = create(:race)
  ...
  it "should save the adjustment when competitor found and valid adjustment" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,2,1')
    check_success 1
  end

  it "should save the adjustment when competitor found and valid negative adjustment" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,2,-2')
    check_success(-2)
  end

  it "should handle error when invalid adjustment" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,2,1.1')
    check_failure
  end

  it "should handle error when unknown leg number" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,1,105')
    check_failure
  end

  it "should handle error when unknown team number" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '4,2,105')
    check_failure
  end

  it "should handle error when invalid string format" do
    @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,2.105')
    check_failure
  end
  ...
  context "when data already stored" do
    ...
    it "should handle error when normal input" do
      @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '15,2,13')
      check_failure true, 2
    end

    it "should override when input starts with ++" do
      @qs = RelayShootingPenaltiesAdjustmentQuickSave.new(@relay.id, '++15,2,-3')
      check_success -3
    end
  end
