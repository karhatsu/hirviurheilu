...
describe Api::V2::Official::ShotsController, type: :api do
  ...
  describe 'PUT (single)' do
    ...
    context 'when race found and correct api key' do
      ...
      context 'but too big shot value given' do
        it 'returns 400' do
          make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots/1", { value: race.sport.best_shot_value + 1 }
          expect_error 400, 'invalid shot value'
        end
      end
  ...
  describe 'PUT (all)' do
    ...
    context 'when race has no API secret' do
      ...
      it 'returns 500' do
        make_request "/api/v2/official/races/#{race2.id}/competitors/#{competitor.number}/shots", body
        expect_status_code 401
      end
    end
    ...
    context 'when race not found' do
      it 'returns 404' do
        make_request "/api/v2/official/races/#{race.id + 10}/competitors/#{competitor.number}/shots", body
        expect_error 404, 'race not found'
      end
    end
    ...
    context 'when race found but wrong api key' do
      it 'returns 401' do
        make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", body, 'wrong-key'
        expect_status_code 401
      end
    end
    ...
    context 'when race found and correct api key' do
      context 'when invalid JSON' do
        it 'returns 400' do
          put "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", '{ "shots": foobar }',
              { 'CONTENT_TYPE' => 'application/json', 'HTTP_AUTHORIZATION' => api_secret }
          expect_error 400, 'invalid JSON'
        end
      end
      ...
      context 'but no shots given' do
        it 'returns 400' do
          make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", {}
      ...
      context 'but shots is not an array' do
        it 'returns 400' do
          make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", { shots: 'foobar' }
      ...
      context 'but invalid shots given' do
        it 'returns 400' do
          make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", { shots: [10, 'hello', 'world'] }
          expect_error 400, 'Laukaukset sisältää virheellisen numeron'
        end
      end

      context 'and valid shots sent' do
        before do
          make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", body
        end
        ...
        context 'and another set of valid shots sent' do
          ...
          it 'overrides the existing shots array' do
            make_request "/api/v2/official/races/#{race.id}/competitors/#{competitor.number}/shots", { shots: new_shots }
