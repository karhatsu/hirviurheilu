module ShotFieldsHelper
  def shot_fields_config(sport, competitor)
    config = Hash.new
    qualification_round_shot_count = sport.qualification_round_shot_count
    final_round = sport.final_round
    final_round_shots_count = final_round && final_round.inject(:+) || 0
    basic_shots_count = qualification_round_shot_count + final_round_shots_count
    shots_per_extra_round = sport.shots_per_extra_round
    config[:qualification_round] = sport.qualification_round
    config[:qualification_round_shot_count] = qualification_round_shot_count
