  ...
  return (
    <>
        ...
        return (
          <div key={id} className="form__horizontal-fields">
            <div className="form__field">
              ...
              />
              <label htmlFor={`series_${id}`}>
                {name} ({competitorsCountLabel(t, competitorsCount)})
              </label>
            </div>
          </div>
        )
      ...
      <div className="form__buttons">
        <Button onClick={start} type="primary" disabled={disabled}>
          {t('resultRotationStart')}
        </Button>
      </div>
    </>
  )
