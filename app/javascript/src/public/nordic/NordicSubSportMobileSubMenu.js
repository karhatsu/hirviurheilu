  ...
  if (nordicSubResultsForSeries) {
    return (
      <div className="buttons buttons--mobile">
        <MobileSubMenu items={race.series} currentId={seriesId} buildPath={buildSeriesPath} parentId={race.id} />
      </div>
    )
  }
    ...
    <div className="buttons buttons--mobile">
      {['trap', 'shotgun', 'rifle_moving', 'rifle_standing'].map((subSport) => {
        ...
        if (currentSubSport === subSport) {
          return (
            <Button key={subSport} type="current">
              {label}
            </Button>
          )
        }
        return (
          <Button key={subSport} to={buildNordicResultsPath(race.id, subSport)}>
            {label}
          </Button>
        )
      })}
