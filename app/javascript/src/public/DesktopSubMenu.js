...
export default function DesktopSubMenu() {
  const { relayId, seriesId, teamCompetitionId, cupSeriesId, cupTeamCompetitionId, rifleCupSeriesId } = usePathParams()
  const { race } = useRace()
  const { cup } = useCup()
  return (
    <Routes>
      <Route path="races/:raceId/*" element={null}>
        ...
        />
        <Route path="relays/:relaysId" element={<RelayDesktopSubMenu race={race} currentRelayId={relayId} />} />
      </Route>
      <Route path="cups/:cupId/*" element={null}>
        ...
        <Route
          ...
          element={<CupTeamCompetitionsDesktopSubMenu cup={cup} currentCupTeamCompetitionId={cupTeamCompetitionId} />}
        />
      </Route>
    </Routes>
  )
