...
export default function RaceTeamCompetitions({ race }) {
  ...
  const buildLink = useCallback(
    (tc, buildPath) => {
      ...
      return (
        <Button key={id} type="primary" to={buildPath(race.id, id)}>
          {linkText}
        </Button>
      )
    },
  ...
  return (
    <>
      ...
      <div className="buttons">{teamCompetitions.map((tc) => buildLink(tc, buildTeamCompetitionsPath))}</div>
      {sport.european && (
        <>
          <h2>{t('rifleTeamCompetitions')}</h2>
          <div className="buttons">{teamCompetitions.map((tc) => buildLink(tc, buildRifleTeamCompetitionsPath))}</div>
        </>
      )}
    </>
  )
