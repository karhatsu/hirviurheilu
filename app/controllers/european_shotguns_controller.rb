class EuropeanShotgunsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.pdf {
        render pdf: "#{@series.race.name}-#{@series.name}-haulikko-tulokset", layout: true, margin: pdf_margin,
               header: pdf_header("#{@series.race.name} - #{@series.name} - #{I18n.t('sport_name.european_sub.shotgun')}\n"),
