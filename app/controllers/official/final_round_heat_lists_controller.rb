class Official::FinalRoundHeatListsController < Official::HeatListsController
  ...
  def create
    ...
    generator.generate_final_round params[:first_heat_number].to_i, params[:first_track_place].to_i, params[:first_heat_time],
                       params[:minutes_between_heats].to_i, params[:competitors_count].to_i, opts
    if generator.errors.empty?
      flash[:success] = t('.heat_list_generated')
      redirect_to official_series_final_round_heat_list_path(@series)
    else
  ...
  def check_heat_count
    if @race.final_round_heats.empty?
  ...
  def find_competitors_without_heat
    @competitors_without_heat = @series.competitors.where('final_round_heat_id IS NULL AND final_round_track_place IS NULL')
  end

  def set_menu
    @final_round_heat_list = true
  end
end
