...
json.(@cup, :id, :name, :public_message, :top_competitions, :include_always_last_race, :use_qualification_round_result)
json.has_rifle @cup.has_rifle?
json.races @cup.races do |race|
  json.(race, :id, :name, :start_date, :end_date)
end
json.cup_series @cup.cup_series do |cup_series|
  ...
  if params[:results]
    json.cup_competitors cup_series.results do |cup_competitor|
      json.(cup_competitor, :first_name, :last_name, :series_names, :score, :club_name)
    end
  end
end
